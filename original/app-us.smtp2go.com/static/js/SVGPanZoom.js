(function (global, factory) {
  ...
  var Animation = function Animation(
    ...
    this.getCurrentState = function () {
      ...
      Object.keys(initialState).forEach(function (key) {
        currentState[key] =
          initialState[key] +
          (now / time) * (finalState[key] - initialState[key]);
      });
      return currentState;
    };
   ...
   */
  var getViewBoxCoordinatesFromEvent = function getViewBoxCoordinatesFromEvent(
    svgRoot,
    e,
  ) {
  ...
  var SVGPanZoom = (function () {
    function SVGPanZoom(svg, options) {
      ...
      if (!(svg instanceof SVGElement)) {
        throw new Error(
          "Invalid Parameters. Firt parameter to SVGPanZoom should be an svg element",
        );
      }
      ...
      setOptions: {
        ...
        this.options = {
          ...
          set limits(value) {
            _limits = {};
            var directionalLimits = (
              (value ? value : value === 0 ? 0 : 15) + ""
            )
              .trim()
              .split(" ");
            ...
            horizontal: {
              var multiplier =
                Number(
                  (directionalLimits[1] || directionalLimits[0]).replace(
                    /%/g,
                    "",
                  ),
                ) / 100;
            ...
            vertical: {
              var _multiplier =
                Number(directionalLimits[0].replace(/%/g, "")) / 100;
      ...
      // Zoom methods
      zoom: {
        var zoomMethod = function zoomMethod(
          callback,
          focalPoint,
          amount,
          animationTime,
        ...
        this.zoomIn = zoomMethod.bind(
          this,
          function (focalPoint, amount, animationTime) {
            return _this.zoomOut(focalPoint, -amount, animationTime);
          },
        );

        this.zoomOut = zoomMethod.bind(
          this,
          function (focalPoint, amount, animationTime) {
            ...
            var minWidthAfterZoom =
              _this.options.initialViewBox.width / _this.options.zoom.maxZoom;
            var maxWidthAfterZoom =
              _this.options.initialViewBox.width / _this.options.zoom.minZoom;
            ...
            var minHeightAfterZoom =
              _this.options.initialViewBox.height / _this.options.zoom.maxZoom;
            var maxHeightAfterZoom =
              _this.options.initialViewBox.height / _this.options.zoom.minZoom;
            ...
            if (!focalPoint) {
              ...
              origin = {
                x:
                  focalPoint.x +
                  (newWidth / viewBox.width) * (viewBox.x - focalPoint.x),
                y:
                  focalPoint.y +
                  (newHeight / viewBox.height) * (viewBox.y - focalPoint.y),
              };
            }

            return _this.setViewBox(
              origin.x,
              origin.y,
              newWidth,
              newHeight,
              animationTime,
              function () {
                _this.options.zoom.callback(
                  _this.options.initialViewBox.width / newWidth,
                  _this.getViewBox(),
                );
              },
            );
    ...
    _createClass(SVGPanZoom, [
      {
        ...
        value: function validateLimits(viewBox) {
          ...
          if (viewBox.width <= initialViewBox.width) {
            viewBox.x = Math.min(
              Math.max(viewBox.x, limits.minX),
              limits.maxX + (initialViewBox.width - viewBox.width),
            );
          } else {
            viewBox.x = Math.min(
              Math.max(
                viewBox.x,
                limits.minX + (initialViewBox.width - viewBox.width),
              ),
              limits.maxX,
            );
          }
          ...
          if (viewBox.height <= initialViewBox.height) {
            viewBox.y = Math.min(
              Math.max(viewBox.y, limits.minY),
              limits.maxY + (initialViewBox.height - viewBox.height),
            );
          } else {
            viewBox.y = Math.min(
              Math.max(
                viewBox.y,
                limits.minY + (initialViewBox.height - viewBox.height),
              ),
              limits.maxY,
            );
          }
        ...
        value: function _setupEvents() {
          ...
          var handlers = {
            mousewheel: function mousewheel(event) {
              if (!this.options.zoom.events.mouseWheel) {
                return;
              }
              ...
              var detail =
                event.detail || event.deltaX || event.deltaY || event.deltaZ;
              var delta = parseInt(-detail || event.wheelDelta);

              if (
                !delta ||
                !this.options.zoom ||
                !this.options.zoom.events.mouseWheel
              ) {
                return;
              }
          ...
          touchEvents: {
            ...
            handlers.pinchAndDrag = function (event) {
              ...
              if (
                !this.options.pan.events.drag ||
                (event.type === "mousedown" &&
                  event.which !== this.options.pan.events.dragMouseButton) ||
                dragStarted ||
                scaleStarted
              ) {
                return;
              ...
              if (this.options.pan.events.dragCursor !== null) {
                this.options.eventMagnet.style.cursor =
                  this.options.pan.events.dragCursor;
              }
              ...
              var mouseMoveCallback = function mouseMoveCallback(event2) {
                ...
                if (
                  Math.sqrt(
                    Math.pow(event.pageX - event2.pageX, 2) +
                      Math.pow(event.pageY - event2.pageY, 2),
                  ) > 25
                ) {
                  preventClick = true;
                }
                ...
                if (dragStarted) {
                  var initialMousePosition = getViewBoxCoordinatesFromEvent(
                    svg,
                    event,
                  );
                  var currentMousePosition = getViewBoxCoordinatesFromEvent(
                    svg,
                    event2,
                  );
                  _this2.pan(
                  ...
                  if (pinchDistance > newPinchDistance) {
                    _this2.zoomOut(
                      mouse,
                      (pinchDistance - newPinchDistance) / newPinchDistance,
                      0,
                    );
                  } else {
                    _this2.zoomOut(
                      mouse,
                      (pinchDistance - newPinchDistance) / pinchDistance,
                      0,
                    );
                  }
              ...
              var mouseUpCallback = function mouseUpCallback(event2) {
                if (
                  (event2.type === "mouseout" &&
                    event2.target !== event2.currentTarget) ||
                  (event2.type === "mouseup" &&
                    event2.which !== _this2.options.pan.events.dragMouseButton)
                ) {
                  return;
                }
          ...
          this.options.eventMagnet.addEventListener(
            "DOMMouseScroll",
            handlers.mousewheel,
          );
          this.options.eventMagnet.addEventListener(
          ...
          this.destroy = function () {
            this.options.eventMagnet.removeEventListener(
              "DOMMouseScroll",
              handlers.mousewheel,
            );
            this.options.eventMagnet.removeEventListener(
