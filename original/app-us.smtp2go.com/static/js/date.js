 ...
 */ (function () {
  ...
  var getText = {
    getFromKey: function (key, countryCode) {
      ...
      if (
        Date.CultureStrings &&
        Date.CultureStrings[countryCode] &&
        Date.CultureStrings[countryCode][key]
      ) {
  ...
  $D.i18n = {
    ...
    setLanguage: function (code, force, cb) {
      ...
      if (
        force ||
        code === "en-US" ||
        (!!Date.CultureStrings && !!Date.CultureStrings[code])
      ) {
        lang = code;
        ...
        if (!(!!Date.CultureStrings && !!Date.CultureStrings[code])) {
          if (typeof exports !== "undefined" && this.exports !== exports) {
            ...
            Date.console.error(
              "The DateJS IETF language tag '" +
                code +
                "' is not available and has not been loaded.",
            );
            return false;
          }
        }
...
(function () {
  ...
  $P.setWeek = function (n) {
    if (n - this.getWeek() === 0) {
      ...
      return this.moveToDayOfWeek(1, this.getDay() > 1 ? -1 : 1).addWeeks(
        n - this.getWeek(),
      );
    }
  };
  ...
  $P.getTimezone = function () {
    return $D.getTimezoneAbbreviation(
      this.getUTCOffset(),
    ...
    return function (m) {
      ...
      switch (m) {
        ...
        case "B":
          ...
          return (
            "@" +
            (context.getUTCSeconds() +
              context.getUTCMinutes() * 60 +
              (context.getUTCHours() + 1) * 3600) /
              86.4
          );
        default:
          return m;
  ...
  $P.toString = function (format, ignoreStandards) {
    ...
    return format
      ? format
          .replace(
            /((\\)?(dd?d?d?|MM?M?M?|yy?y?y?|hh?|HH?|mm?|ss?|tt?|S|q|Q|WW?W?W?)(?![^\[]*\]))/g,
            parseFormatStrings,
          )
          .replace(/\[|\]/g, "")
      : this._toString();
...
(function () {
  ...
  $N.ago = $N.before = function (date) {
    var c = {},
      s =
        this._dateElement[this._dateElement.length - 1] !== "s"
          ? this._dateElement + "s"
          : this._dateElement;
    c[s] = this * -1;
    return (!date ? new Date() : date.clone()).add(c);
  ...
  var dx = "sunday monday tuesday wednesday thursday friday saturday".split(
    ...
    px =
      "Millisecond Second Minute Hour Day Week Month Year Quarter Weekday".split(
        /\s/,
      ),
    pxf =
      "Milliseconds Seconds Minutes Hours Date Week Month FullYear Quarter".split(
        /\s/,
      ),
  ...
  var processTerms = function (names, staticFunc, instanceFunc) {
    for (var i = 0; i < names.length; i++) {
      ...
      $D[names[i].toUpperCase()] = $D[names[i].toUpperCase().substring(0, 3)] =
        i;
...
(function () {
  ...
  var utils = {
      ...
      for (key in hash) {
        if (Object.prototype.hasOwnProperty.call(hash, key)) {
          ...
          if (typeof hash[key] === "function") {
            ...
            regex =
              hash[key] instanceof RegExp
                ? hash[key]
                : new RegExp(hash[key], "g");
          }
    ...
    getDayOfYearFromWeek: function (obj) {
      ...
      offset = jan4 + 3;
      obj.dayOfYear =
        obj.week * 7 + (obj.weekDay === 0 ? 7 : obj.weekDay) - offset;
      return obj;
    },
    ...
    adjustForTimeZone: function (obj, date) {
      var offset;
      if (
        obj.zone.toUpperCase() === "Z" ||
        (obj.zone_hours === 0 && obj.zone_minutes === 0)
      ) {
  ...
  $P.Numeric = {
    ...
    parse: function (s) {
      ...
      if (
        !this.isNumeric(s) || // if it's non-numeric OR
        (s[0] === "+" && s[0] === "-")
      ) {
  ...
  $P.Normalizer = {
    ...
    buildRegexFunctions: function () {
      var $R = Date.CultureInfo.regexPatterns;
      var __ = Date.i18n.__;
      var tomorrowRE = new RegExp(
        "(\\b\\d\\d?(" +
          __("AM") +
          "|" +
          __("PM") +
          ")? )(" +
          $R.tomorrow.source.slice(1) +
          ")",
        "i",
      ); // adapted tomorrow regex for AM PM relative dates
    ...
    buildReplaceData: function () {
      ...
      this.replaceHash = utils.addToHash(
        this.basicReplaceHash(),
        this.keys(),
        this.regexData(),
      );
    },
  ...
  $D.Translator = {
    ...
    finish: function (x) {
      ...
      if (
        ...
        "year day hour minute second".indexOf(this.unit) !== -1
      ) {
        if (!this.value) {
          this.value = this.month + 1;
        }
      ...
      if (this.timezone) {
        ...
        if (today.hasDaylightSavingTime()) {
          ...
          timezone = $D.getTimezoneAbbreviation(
            offset,
...
(function () {
  ...
  var cacheProcessRtoken = function (key, token, type, eachToken) {
    if (eachToken) {
      g[key] = _.cache(
        _.process(
          _.each(_.rtoken(token), _.optional(g.ctoken2(eachToken))),
          type,
        ),
      );
    } else {
...
(function () {
  ...
  var TimeSpan = function (days, hours, minutes, seconds, milliseconds) {
    ...
    this.add = function (time) {
      return time === null
        ? this
        : this.addSeconds(time.getTotalMilliseconds() / 1000);
    };
    ...
    this.toString = function (format) {
      ...
      return format
        ? format.replace(/dd?|HH?|hh?|mm?|ss?|tt?/g, function (format) {
            switch (format) {
              case "d":
                return me.getDays();
              case "dd":
...
(function () {
  ...
  var adjustForDST = function (orient, startDate, endDate) {
    var hasDSTMismatch =
      false ===
      (startDate.isDaylightSavingTime() === endDate.isDaylightSavingTime());
  ...
  TimePeriod.prototype.set = function (
    years,
    months,
    days,
    hours,
