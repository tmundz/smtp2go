...
(function ($) {
  function labelAxis(val, axis) {
    ...
    var tmpopts =
      axis.n == 1
        ? opts
        : typeof opts.alignedTo != "undefined"
          ? opts.alignedTo.options
          : null;
    // first axis or some axis aligned wrt it
    if (
      tmpopts &&
      (tmpopts.autoscaleMargin == null ||
    ...
    if (
      ...
      if ($.isFunction(opts.userFormatter)) {
        ...
        if (
          (axis.datamax != 0 && Math.abs(axis.datamax) < 1e-5) ||
  ...
  function init(plot) {
    plot.hooks.processOptions.push(function (plot, options) {
      // separate X and Y
      $.each(
        { x: options.xaxes, y: options.yaxes },
        function (direction, axes) {
          // get only axes with labels
          $.each(
            $.grep(axes, function (v) {
  ...
  var options = {
    xaxis: { label: null, labelPos: "high" },
